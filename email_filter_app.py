...
<head>
  ...
  <style>
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    /* Tiny separators to clarify sections */
...
def _matches_by_mode(full_text: str, phrases: list[str], require_all: bool) -> bool:
    ...
    if require_all:
        ...
        for p in phrases:
            if not _phrase_in_text(full_text, p):
                return False
        return True
    else:
        # ANY phrase present
        for p in phrases:
            if _phrase_in_text(full_text, p):
                return True
        return False
...
def process_file():
    try:
        ...
        require_all = bool(data.get("require_all", False))  # NEW: ANY vs ALL switch
        ...
        for _, row in df.iterrows():
            ...
            # Sender scan (still ANY match of important senders)
